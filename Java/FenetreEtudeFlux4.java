...
public class FenetreEtudeFlux4 extends JFrame
{
	...
	private String sousSousDossier = "Bd-Flux\\";
	...
	private String[] englishNameTerritoire2 =  null;
	private String[] nomFrancaisTerritoire2 = null;
	private String[] nomContinent2 = null;
	private String[] anneeDebutTerritoire2 = null;
	private String[] anneeFinTerritoire2 = null;
	...
	private String labelTerritoire2 = "";
	...
	public FenetreEtudeFlux4(String text1, String text2, String[] text3, String[] text4, int anneeDebut, int anneeFin, String[] text5)
	{
...
:			{
		...
		*/
		...
		this.setTitle("\u00c9tude des flux entre territoires : Extraction");
...
//***********************
	private JPanel zoneMouvante()
	{
		...
		//Gestion de l'objet : Choix des territoires d'origine
		panel1.add(new Label("Choix des territoires (Origine) : "));
		...
		//Gestion de l'objet : Choix des territoires de destination
		panel1.add(new Label("Choix des territoires (Destination) : "));
		...
		panel1.add(new JScrollPane(zoneDeTexteChoixTerritoires2));
		//Gestion des doublons
		ArrayList<Integer> posDouble = new ArrayList<Integer>();
		for(int i = 0 ; i < getTerritoire().length ; ++i)
		{
			for(int j = 0 ; j < getTerritoire2().length ; ++j)
			{
				if(getTerritoire()[i].equals(getTerritoire2()[j]))
				{
					posDouble.add(i);
				}
			}
		...
		if(posDouble.size() != 0)
		{
			String[] codeTerritoire = new String[posDouble.size()];
			String[] nomTerritoire = new String[posDouble.size()];
			for(int i = 0 ; i < codeTerritoire.length ; ++i)
			{
				codeTerritoire[i] = getTerritoire()[posDouble.get(i)];
			}
			...
			for(int i = 0 ; i < codeTerritoire.length ; ++i)
			{
				...
				{
					label = label + nomTerritoire[i] + " (" + codeTerritoire[i] + ")\n";
				}
				else
				{
					label = label + nomTerritoire[i] + " (" + codeTerritoire[i] + ")";
			...
			JPanel panel1b = new JPanel(new GridLayout(2, 1));
			panel1b.add(new Label("Attention ! Les territoires suivants sont cit\u00e9s deux fois (origine et destination)."));
			panel1b.add(new Label("Ils ne peuvent \u00eatre crois\u00e9s entre eux."));
			panel1.add(panel1b);
			JTextArea zoneDeTexteChoixDouble = new JTextArea(label);
			zoneDeTexteChoixDouble.setEditable(false);//Interdire la modification du texte de la zone
			zoneDeTexteChoixDouble.setBackground(new Color(240,240,240));//Couleur du fond
			zoneDeTexteChoixDouble.setForeground(Color.GRAY.brighter());//Couleur du texte
			panel1.add(new JScrollPane(zoneDeTexteChoixDouble));
		}
		else
		{
			panel1.add(new Label("Aucun doublon !"));
...
//*************************************
	private void tableauExtrait()
	{
		...
		fenetreDonnees.setTitle("\u00c9tude des flux entre territoires : Extraction");
	...
	{
		String[] codeTerritoire = new String[getTerritoire2().length];
		String[] nomTerritoire = new String[getTerritoire2().length];
		for(int i = 0 ; i < getTerritoire2().length ; ++i)
		{
			codeTerritoire[i] = getTerritoire2()[i];
		}
		ArrayList<Integer> positionNomTerritoire = new ArrayList<Integer>();
		for(int i = 0 ; i < codeTerritoire.length ; ++i)
		{
		...
		this.labelTerritoire2 = labelTerritoire2;
	}
...
//*******************************************
	private void init()
	{
		...
		//Chargement des données attributaires des territoires sélectionnés
		englishNameTerritoire2 = new String[getTerritoire2().length];
		nomFrancaisTerritoire2 = new String[getTerritoire2().length];
		nomContinent2 = new String[getTerritoire2().length];
		anneeDebutTerritoire2 = new String[getTerritoire2().length];
		anneeFinTerritoire2 = new String[getTerritoire2().length];
		ArrayList<Integer> position2 = new ArrayList<Integer>();
		for(int i = 0 ; i < getTerritoire2().length ; ++i)
		{
			for(int j = 0 ; j < getBaseAttributaireTerritoireCodeISO().length ; ++j)
			{
				if(getTerritoire2()[i].equals(getBaseAttributaireTerritoireCodeISO()[j]) == true)
				{
					position2.add(j);
				}
			}
		...
		{
			englishNameTerritoire2[i] = getBaseAttributaireTerritoireEnglishName()[position2.get(i)];
			nomFrancaisTerritoire2[i] = getBaseAttributaireTerritoireNomFrancais()[position2.get(i)];
			nomContinent2[i] = getBaseAttributaireTerritoireNomContinent()[position2.get(i)];
			anneeDebutTerritoire2[i] = getBaseAttributaireTerritoireBorneAnneeDebut()[position2.get(i)];
			anneeFinTerritoire2[i] = getBaseAttributaireTerritoireBorneAnneeFin()[position2.get(i)];
		}
		setEnglishNameTerritoire2(englishNameTerritoire2);
		setNomFrancaisTerritoire2(nomFrancaisTerritoire2);
		setNomContinent2(nomContinent2);
		setAnneeDebutTerritoire2(anneeDebutTerritoire2);
		setAnneeFinTerritoire2(anneeFinTerritoire2);
	}
		...
		else
		{
			...
			else
			{
				if(getChoixDesNiveauxInt() == 4)
				{
					...
					setBaseAttributaireProduitNomProduit(baseAttributaireProduit.getNom4Digit());
				...
				else
				{
					if(getChoixDesNiveauxInt() == 6)
					{
						...
						setBaseAttributaireProduitNomProduit(baseAttributaireProduit.getNom6Digit());
	...
	private void chargementFichierALire()
	{
		...
		for(int i = 0 ; i < getAnnee().length ; ++i)
		{
			for(int j = 0 ; j < getTerritoire().length ; ++j)
			{
				...
				{
					String adresse2 = adresse + getAnnee()[i] + "\\" + getTerritoire()[j] + ".csv";
...
//			listeData.affichageDonnees();//Afficher les données de la base
			...
			ArrayList<Integer> positionSupp = new ArrayList<Integer>();
			for(int i = 0 ; i < data.length ; ++i)
			{
				for(int j = 0 ; j < getTerritoire2().length  ; ++j)
				{
					if(data[i][3].equals(getTerritoire2()[j]))
					{
						positionSupp.add(i);
					}
				}
			}
			String[][] dataSortie2 = new String[positionSupp.size()][numColonne];
			for(int i = 0 ; i < positionSupp.size() ; ++i)
			{
				for(int j = 0 ; j < numColonne ; ++j)
				{
					dataSortie2[i][j] = data[positionSupp.get(i)][j];
				}
			}
			//Récupération de la colonne du code ISO des territoires d'origine
			String[] codeISO = new String[dataSortie2.length];
			for(int i = 0 ; i < dataSortie2.length ; ++i)
			{
				codeISO[i] = dataSortie2[i][1];
			}
			...
			{
				dataSortie2[i][0] = getBaseAttributaireTerritoireNomFrancais()[positionISO.get(i)];
			}
			//Récupération de la colonne du code ISO des territoires de destination
			String[] codeISO2 = new String[dataSortie2.length];
			for(int i = 0 ; i < dataSortie2.length ; ++i)
			{
				codeISO2[i] = dataSortie2[i][3];
			}
			ArrayList<Integer> positionISO2 = new ArrayList<Integer>();
			for(int i = 0 ; i < codeISO2.length ; ++i)
			{
				for(int j = 0 ; j < getBaseAttributaireTerritoireCodeISO().length ; ++j)
				{
					if(codeISO2[i].equals(getBaseAttributaireTerritoireCodeISO()[j]))
					{
						positionISO2.add(j);
					}
				}
			}
			for(int i = 0 ; i < positionISO2.size() ; ++i)
			{
				dataSortie2[i][2] = getBaseAttributaireTerritoireNomFrancais()[positionISO2.get(i)];
			}
			//Récupération de la colonne du code des produits
			String[] codeProduit = new String[dataSortie2.length];
			for(int i = 0 ; i < dataSortie2.length ; ++i)
			{
				codeProduit[i] = dataSortie2[i][6];
			}
			ArrayList<Integer> position = new ArrayList<Integer>();
			for(int i = 0 ; i < codeProduit.length ; ++i)
			{
			...
			for(int i = 0 ; i < position.size() ; ++i)
			{
				for(int j = 0 ; j < numColonne ; ++j)
				{
					dataSortie[i][j] = dataSortie2[position.get(i)][j];
				}
			}
			...
			for(int i = 0 ; i < dataSortie.length ; ++i)
			{
				for(int j = 0 ; j < getBaseAttributaireProduitCodeProduit().length ; ++j)
				{
					if(dataSortie[i][6].equals(getBaseAttributaireProduitCodeProduit()[j]))
					{
						positionNomProduit.add(j);
					}
				}
			...
			for(int i = 0 ; i < positionNomProduit.size() ; ++i)
			{
				dataSortie[i][7] = getBaseAttributaireProduitNomProduit()[positionNomProduit.get(i)];
			}
...
//**************************************
	private void affichageDonneesRecuperees()
	{
		...
		System.out.print("Liste des territoires originaires retenus : ");
		for(int i = 0 ; i < getTerritoire().length ; ++i)
		{
		...
		System.out.print("Liste des territoires destinataires retenus : ");
		for(int i = 0 ; i < getTerritoire2().length ; ++i)
		{
			if(i < getTerritoire2().length - 1)
			{
				System.out.print(getTerritoire2()[i] + ",");
			}
			else
			{
				System.out.print(getTerritoire2()[i] + "\n");
...
//************************
	class ClicRetour implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			{
				FenetreEtudeFlux3 renvoiDonnees = new FenetreEtudeFlux3(getChoixDesClassifications(), getChoixDesNiveaux(), getTerritoire(), getTerritoire2(), getAnneeDebut(), getAnneeFin(), getProduit());
	...
	class ClicSuivant implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			String nomDuFichier = "Extraction-Flux-" + getChoixDesClassifications() + "-" + getChoixDesNiveaux();
			ecrire(nomDuFichier);
